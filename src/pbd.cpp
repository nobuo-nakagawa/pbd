...
struct Context {
  ...
  int                 num_iteration;
  Float               stiffness;
  Context() : frame(0), time(0.0f), debug_info(), floor(), light(), floor_shadow(), scene(nullptr), num_iteration(20), stiffness((Float)0.1) {}
};
...
public:
  ...
  DistanceConstraint(Point* p0, Point* p1, Float in_stiffness) : point0(p0), point1(p1), rest_length((Float)0.0), stiffness(in_stiffness) {
  ...
  void SolvePosition(Float dt){
    ...
    grad        /= d; // normalize
    Vec3  corr = stiffness * grad * (d - rest_length) / w; // delta P
...
private:
  ...
  void   MakeConstraint(Point* p1, Point* p2, Float stiffness) { constraints.push_back(DistanceConstraint(p1, p2, stiffness)); }
  void   CalcNormal() {
    normals.clear();
...
public:
  SceneCloth(Vec2& width, glm::ivec2& in_div, Vec3& in_pos, Float in_stiffness) : size(in_div.x, in_div.y), points(), normals(), constraints() {
    ...
    for(int w = 0; w < size.x; w++){
      for(int h = 0; h < size.y; h++){               // structual constraint
        if  (w < size.x - 1){ MakeConstraint(GetPoint(w, h), GetPoint(w+1, h  ), in_stiffness); }
        if  (h < size.y - 1){ MakeConstraint(GetPoint(w, h), GetPoint(w,   h+1), in_stiffness); }
        if ((w < size.x - 1) && (h < size.y - 1) ) { // shear constraint
          MakeConstraint(GetPoint(w,   h), GetPoint(w+1, h+1), in_stiffness);
          MakeConstraint(GetPoint(w+1, h), GetPoint(w,   h+1), in_stiffness);
        }
      }
    }
    for(int w = 0; w < size.x; w++){
      for(int h = 0; h < size.y; h++){               // bend constraint
        if  (w < size.x  - 2){ MakeConstraint(GetPoint(w, h), GetPoint(w+2, h  ), in_stiffness); }
        if  (h < size.y  - 2){ MakeConstraint(GetPoint(w, h), GetPoint(w,   h+2), in_stiffness); }
        if ((w < size.x  - 2) && (h < size.y - 2)) {
          MakeConstraint(GetPoint(w,   h), GetPoint(w+2, h+2), in_stiffness);
          MakeConstraint(GetPoint(w+2, h), GetPoint(w,   h+2), in_stiffness);
        }
      }
    }
    CalcNormal();
  ...
  virtual void Render(float alpha = 1.0f) {
    ...
    for(int w = 0; w < size.x - 1; w++){
      for(int h = 0; h < size.y - 1; h++){
#if 0
        glm::vec4 diffuse(1.0f, 0.6f, 0.6f, alpha);
        glm::vec4 ambient(0.3f, 0.3f, 0.3f, alpha);
        if ( col_idx++ % 2 ){ diffuse = glm::vec4(1.0f, 1.0f, 1.0f, alpha);}
        glMaterialfv(GL_FRONT, GL_DIFFUSE,  (GLfloat*)&diffuse);
        glMaterialfv(GL_FRONT, GL_AMBIENT,  (GLfloat*)&ambient);
#else
        set_material(mat_ruby,  alpha, GL_FRONT);
        set_material(mat_pearl, alpha, GL_BACK);
#endif
...
//  glHint(GL_POLYGON_SMOOTH_HINT, GL_NICEST);
  ...
  g_Context.scene = new SceneCloth(Cloth::WIDTH, Cloth::DIVISION, Cloth::POS, g_Context.stiffness);
}
...
void display_imgui() {
...

  {
    ...
    ImGui::SliderInt  ("Iterations", &g_Context.num_iteration, 1, 1000);
    if (ImGui::SliderFloat("Stiffness",  &g_Context.stiffness,     0.01f, 1.0f)) {
      restart();
    }
    ImGui::End();
  }
